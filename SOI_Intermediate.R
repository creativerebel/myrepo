...
ui <- function(input, output, session)
{
  navbarPage(title = div("SOI Automation",img(src = "https://fractalanalytics.com/wp-content/uploads/2018/03/Fractal-Logo-WithBL.png", height = "32px",
             ...
             #================================================================Data Review Tab=======================================================================
             tabPanel("Data Review",tabsetPanel(type ="tabs",
                                                tabPanel("Category Level",sidebarLayout(
                                                  sidebarPanel(
                                                    ...
                                                    selectInput("target","Target Variable:",choices = c("Volume" = "ALWAYS.VOL",
                                                                                                        "Value" = "ALWAYS.VAL")),
                                                    selectInput("split","Split Layout by:",choices = c("None","Area")),
                                                    selectInput("lineby","Line by:",choices = c("None","Area"))
                                                    ,width = 2),
                                                  mainPanel(
...
server <- function(input, output, session) {
  ...
  p <- reactive({
    req(input$split)
    plot <- ggplot(data = category_subset(), aes_string(x = "Month", y = input$target)) + 
    ...
    if("Area" %in% input$split) return (plot + facet_grid(~input$area))
    if("None" %in% input$split) return (plot)
